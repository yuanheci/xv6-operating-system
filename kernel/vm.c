...
int
kpmappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  uint64 a, last;
  pte_t *pte;
...
kpfreewalk(pagetable_t pagetable)
{
  ...
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){  //有子页表  
      // this PTE points to a lower-level page table.
      uint64 child = PTE2PA(pte);
...
int
kpvmcopy(pagetable_t old, pagetable_t new, uint64 begin, uint64 end)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  begin = PGROUNDDOWN(begin);  //4k对齐
  ...
  for(i = begin; i < end; i += PGSIZE){
    ...
    if(kpmappages(new, i, PGSIZE, pa, flags) != 0){
      goto err;
    }
  }
  return 0;
